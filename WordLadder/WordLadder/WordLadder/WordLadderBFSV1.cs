...
public class SolutionBVSV1
{
    ...
    public int LadderLength(string beginWord, string endWord, IList<string> wordList)
    {
        ...
        LadderLength(transformationStack);
        return minLength == int.MaxValue ? 0 : minLength;
    }

    private void LadderLength(Stack<string> transformationStack)
    {
        while (queue.Count > 0)
        {
            string currentWord = queue.Dequeue();
            if (IsOneLetterDifferent(currentWord, transformationStack.Peek()))
            {
                transformationStack.Push(currentWord);
            ...
            for (int i = 0; i < wordList.Count; i++)
            {
                ...
                if (!transformationStack.Contains(iterWord) && IsOneLetterDifferent(iterWord, transformationStack.Peek()))
                {
                    Print($"Found word: {iterWord} which is 1 letter different to current word {transformationStack.Peek()}.");
            ...
            if (!wasEnqueued)
                if (transformationStack.Count > 1)
